...
//$Id: CmsSettingsForm.inc.php,v 1.14 2008/07/01 01:16:13 asmecher Exp $
...
class CmsSettingsForm extends Form {
	...
	function checkAndFixContent ( $content ) {
		...
		if ( strlen($content) > 0 &&
			...
			$this->addError('content', __('plugins.generic.cms.error.muststartwithheader'));
			return true;
		}
		...
		if ( count($headMatches[0]) != count($contentMatches[0]) ) {
			...
			$this->addError('content', __('plugins.generic.cms.error.musthavecontent'));
			return true;
		} 
		...
		for ( $i = 0; $i < count($headMatches[0]); $i++ ) {
			...
			if ( str_replace(' ', '', $headMatches[2][$i]) == '' ) {
				$this->setData('content', $content);
				$this->addError('content', __('plugins.generic.cms.error.headingcannotbeempty'));
				return true;				
			}			
		}
